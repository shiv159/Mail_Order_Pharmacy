...
/*
	@Test
	 void validateTestValidtoken() {
		...
		when(jwtutil.extractUsername("token")).thenReturn("chandu");
		UserData user1 = new UserData("chandu", "chandu", "chandu",null);
		Optional<UserData> data = Optional.of(user1);
		when(userservice.findById("chandu")).thenReturn(data);
